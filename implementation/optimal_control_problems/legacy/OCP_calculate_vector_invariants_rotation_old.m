classdef OCP_calculate_vector_invariants_rotation_old < handle
    ...
    methods
        function obj = OCP_calculate_vector_invariants_rotation_old(parameters)
            ...
            for k=1:window_length
                ...
                objective_fit = objective_fit + weight_accuracy*dot(e_rotation,e_rotation); % apply weighting to error
            end
        ...
        function optimization_result = calculate_invariants(obj,meas_Obj_frames,stepsize)
            ...
            %             [FS_init,~,invariants_init] = calculate_vector_invariants_from_discrete_twist(twist_init,stepsize,parameters);
            %             invariants_init = invariants_init(:,1:3) +1e-10;
            %L_init = 1;


            end_ind = round(N/2,0);
            ASA = calculate_ASA_pose(twist_init(1:end_ind,:));
            ...
            for k=1:N
                obj.opti.set_initial(obj.X.R_FS{k}, ASA(1:3,1:3)); %construct_init_FS_from_traj(meas_traj.Obj_location);
            ...
            optimization_result.Obj_location = reshape(solution.value([obj.X.R_obj{:}]),3,3,N);
