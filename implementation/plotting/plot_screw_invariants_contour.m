function plot_screw_invariants_contour(progress_ref,invars_ref,progress,invars_all,datatype)
...
for i=1:2 % top row, bottom row
    for j=1:3 % three columns
        ...
        % dummy plots for the first invariant so that -1 and +1 are included in the Y-axis
        ...
        title(label_y{3*(i-1)+j},'Interpreter','LaTex','FontSize',20,'Rotation',0,'HorizontalAlignment', 'center');
    end
end
