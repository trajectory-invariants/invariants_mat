function plot_FS_frames_contour(invariant_frame, object_invariant, pose, trial, viewpoint, referencepoint, datatype, parameterization)

figure('Name',['FS frames of ',datatype],'Color',[1 1 1]); hold on; axis equal;
...
% ISA axis
arrow_length_isa = 0.2; arrow_width_isa = 0.0005*6; axis_width_isa = 2;
axis_length_isa = 0.3; view_fig = [-50,10]; step_plot = 2;
    ...
    if ~strcmp(trial,'ref') % not the reference case
        for j = 1 : k_1-1
            ...
            if strcmp(referencepoint,'tracker')
                plot_stl(['data/stl/configuration-',sensor_id(trial),'/tool-',sensor_id(trial),'-print1.STL'],rotation_tcp_k(:,:,j),position_tcp_k(j,:),0.001,part1_color,1);
                plot_stl(['data/stl/configuration-',sensor_id(trial),'/tool-',sensor_id(trial),'-print2.STL'],rotation_tcp_k(:,:,j),position_tcp_k(j,:),0.001,part2_color,1);
                plot_stl(['data/stl/configuration-',sensor_id(trial),'/tool-',sensor_id(trial),'-print3.STL'],rotation_tcp_k(:,:,j),position_tcp_k(j,:),0.001,part3_color,1);
                plot_stl(['data/stl/configuration-',sensor_id(trial),'/tool-',sensor_id(trial),'-sensor.STL'],rotation_tcp_k(:,:,j),position_tcp_k(j,:),0.001,sensor_color,1);
                plot_stl(['data/stl/configuration-',sensor_id(trial),'/tool-',sensor_id(trial),'-tracker.STL'],rotation_tcp_k(:,:,j),position_tcp_k(j,:),0.001,tracker_color,1);
            end
        end
    end
...
if strcmp(viewpoint,'body')
    ...
    elseif ~strcmp(trial,'ref') % not the reference case
        plot_stl(['data/stl/configuration-',sensor_id(trial),'/tool-',sensor_id(trial),'-bearings.STL'],R_visual,zeros(1,3),0.001,bearings_color,0.4);
        plot_stl(['data/stl/configuration-',sensor_id(trial),'/tool-',sensor_id(trial),'-print4.STL'],R_visual,zeros(1,3),0.001,part4_color,0.4);
        if strcmp(referencepoint,'tracker') && ~strcmp(trial,'ref') % not the reference case
            plot_stl(['data/stl/configuration-',sensor_id(trial),'/tool-',sensor_id(trial),'-print1.STL'],R_visual,zeros(1,3),0.001,part1_color,0.4);
            plot_stl(['data/stl/configuration-',sensor_id(trial),'/tool-',sensor_id(trial),'-print2.STL'],R_visual,zeros(1,3),0.001,part2_color,0.4);
            plot_stl(['data/stl/configuration-',sensor_id(trial),'/tool-',sensor_id(trial),'-print3.STL'],R_visual,zeros(1,3),0.001,part3_color,0.4);
            plot_stl(['data/stl/configuration-',sensor_id(trial),'/tool-',sensor_id(trial),'-sensor.STL'],R_visual,zeros(1,3),0.001,sensor_color,0.4);
            plot_stl(['data/stl/configuration-',sensor_id(trial),'/tool-',sensor_id(trial),'-tracker.STL'],R_visual,zeros(1,3),0.001,tracker_color,0.4);
        end
    end
